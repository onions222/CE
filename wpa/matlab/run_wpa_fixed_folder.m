% RUN_WPA_FIXED_FOLDER MATLAB 脚本入口。
%
% 用法：
% 1. 打开本文件
...
% - 会处理常见图像扩展名：png / jpg / jpeg / bmp / tif / tiff
% - 输出文件名自动追加 `_waN` 后缀
%
% 参数说明：
% - input_dir: 输入图像文件夹，只读取当前目录下一层图像文件
% - output_dir: 输出文件夹。若不存在会自动创建
% - wa_sel: 色温调节档位，范围 0~127
% - coeff_frac_bits: 增益 raw code 小数位数。默认 8，对应 COEFF_ONE = 256
% - frac_bits: 像素 raw code 小数位数。默认 10，对应 ONE = 1024
% - gamma_mode: gamma 模式，默认 'srgb'
% - gamma_power: 当 gamma_mode='power' 时使用的 gamma 指数
% - file_patterns: 扫描的文件扩展名模式
%
% WA_SEL 常用预设：
% - 0 = 最暖，warm 端最强
% - 32 = 偏暖，适合中等暖白验证
